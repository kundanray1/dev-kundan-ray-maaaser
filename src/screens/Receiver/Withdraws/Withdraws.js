...
import WithdrawsMedium from "./WithdrawsMedium";
import WithdrawsType from "./WithdrawsType";
...
const Withdraws = ({
  ...
  const [withdrawsType, setWithdrawsType] = useState();
  ...
  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    withdraws(loginData.user.account.accountid);
    setRefreshing(false);
  });
  ...
  useEffect(() => {
    if (data.withdraws == null) {
      withdraws(loginData.user.account.accountid);
    } else {
      const sortedData = Object.values(
        data.withdraws.reduce((acc, item) => {
