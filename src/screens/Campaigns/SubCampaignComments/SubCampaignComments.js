...
const SubCampaignComments = ({
  data,
  loginData,
  subCampaignComments,
  postSubCampaignComments,
  updateSubCampaignComments,
  deleteSubCampaignComments,
  subCampaignId,
}) => {
  ...
  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    subCampaignComments(subCampaignId);
    setRefreshing(false);
  });

  useEffect(() => {
    if(data.postSubCampaignComments!=null){
    if(data.postSubCampaignComments.success){
      setComment("")
    }
  }
    subCampaignComments(subCampaignId);
  }, [subCampaignId,
    data.postSubCampaignComments,
    data.updateSubCampaignComments,
    data.deleteSubCampaignComments,
  ]);

  const onSubmitDonateConfirmation = () => {
    const commentProtoData = new CampaignProto.Comment();
    commentProtoData.setDescription(comment);
    commentProtoData.setRefid(subCampaignId);
    postSubCampaignComments(commentProtoData);
  };
  ...
  const onSubmitDonateUpdateConfirmation = () => {
    ...
    updateSubCampaignComments(commentProtoData);
  };

  const onSubmitDeleteConfirmation = () => {
    deleteSubCampaignComments(commentData.commentid);
  };
  const handleDelete = () => {
    Alert.alert(
      "Comment Deletion",
      ...
      {data.isLoading ? (
        ...
        <>
          <Block style={{ flex: 0, marginTop: 6 }}>
            <FlatList
              data={data.subCampaignComments.commentsList}
...
export default SubCampaignComments;
